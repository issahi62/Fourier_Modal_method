function [R,T,r,t,ampl,z]=stack_tm(theta,ni,n,d,nt);

%function [R,T,r,t,ampl,z]=stack_tm(theta,ni,n,d,nt);
%
%Reflection and transmission from a stack of
%uniform homogeneous layers for TM-polarization.
%
...
kiz=2*pi*cos(theta)/ni;
ktz=2*pi*sqrt(nt^2-ni^2*sin(theta)^2)/nt^2;
gamma=i*sqrt(n.^2-ni^2*sin(theta)^2)./n.^2;
kgd=-2*pi*gamma.*n.^2.*d;
expkgd=exp(-kgd);
exp2kgd=exp(-2*kgd);
